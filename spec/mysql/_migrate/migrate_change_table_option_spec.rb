...
describe 'Ridgepole::Client#diff -> migrate' do
  context 'when change mysql table options' do
    let(:actual_dsl) do
      erbh(<<-ERB)
        create_table "employees", primary_key: "emp_no", <%= i cond("< 6.1", options: "ENGINE=MyISAM DEFAULT CHARSET=utf8") %>, <%= i cond(">= 6.1", charset: "utf8", options: "ENGINE=MyISAM") %>, force: :cascade do |t|
    ...
    let(:expected_dsl) do
      erbh(<<-ERB)
        create_table "employees", primary_key: "emp_no", <%= i cond("< 6.1", options: "ENGINE=InnoDB DEFAULT CHARSET=ascii") %>, <%= i cond(">= 6.1", charset: "ascii") %>, force: :cascade do |t|
          t.date   "birth_date", null: false
