...
describe 'Ridgepole::Client#diff -> migrate' do
  context 'when change mysql table options' do
    let(:actual_dsl) do
      erbh(<<-ERB)
        create_table "employees", <%= i table_options(primary_key: "emp_no", charset: 'utf8', force: :cascade, options: "ENGINE=MyISAM DEFAULT CHARSET=utf8") %> do |t|
    ...
    let(:expected_dsl) do
      erbh(<<-ERB)
        create_table "employees", <%= i table_options(primary_key: "emp_no", charset: 'utf8', force: :cascade, options: "ENGINE=InnoDB DEFAULT CHARSET=ascii") %> do |t|
          t.date   "birth_date", null: false
